...
import random
root = Tk()
root.title('Крестики - Нолики') # Создали окно
game_run = True
field = [] # Двумерный список для хранения кнопки игрового поля
cross_count = 0 # Переменная для отслеживания крестиков на поле

# Функция вызывается при нажатии кнопки начала новой игры
def new_game():
    for row in range(3):
        for col in range(3):
            field[row][col]['text'] = ' '
...
def click(row, col):
    if game_run and field[row][col]['text'] == ' ':
        ...
        if game_run and cross_count < 5:
            computer_move() # Если не победили, то выполняет код комп
            check_win('O')    
